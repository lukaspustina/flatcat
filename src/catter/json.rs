...
impl<'a> JsonCatter<'a> {
    ...
    fn json(&self, json: serde_json::Value) -> Result<()> {
        let mut path = String::new();

        self.do_json(&mut path, json)
    }

    fn do_json(&self, path: &mut String, json: serde_json::Value) -> Result<()> {
        match json {
...
impl<'a> Catter for JsonCatter<'a> {
    fn cat<R: Read>(&self, read: &mut R) -> Result<()> {
        let json: serde_json::Value = serde_json::de::from_reader(read)?;
